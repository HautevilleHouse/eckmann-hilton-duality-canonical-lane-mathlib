...
theorem eckmann_hilton_comm (M : Type u) (d : EckmannHiltonData M) : ∀ a b : M, d.mul1 a b = d.mul1 b a := by
  ...
  calc
    ...
    _ = d.mul2 b a := h_eq'
    _ = d.mul2 a b := by
      have h_comm2 : ∀ x y : M, d.mul2 x y = d.mul2 y x := by
        intro x y
        calc
          d.mul2 x y = d.mul1 x y := (eckmann_hilton_eq M d x y).symm
          _ = d.mul1 y x := by
            have : d.mul1 x y = d.mul2 x y := eckmann_hilton_eq M d x y
            have : d.mul1 y x = d.mul2 y x := eckmann_hilton_eq M d y x
            calc
              d.mul1 x y = d.mul2 x y := this
          ...
          _ = d.mul2 y x := (eckmann_hilton_eq M d y x).symm
      exact h_comm2 a b
